...
def parse_input(line):
    try:
        yield csv.reader([line]).next()
    except:
        yield beam.pvalue.SideOutputValue('error', line)
...
def configure_pipeline(p, opt):
  ...
  results = (p
       ...
       | 'Parse input' >> beam.FlatMap(parse_input).with_outputs('error', main='good'))
  good_input = results.good
  bad_input = results.error
  bad_input | WriteToText('gs://bugoff-bucket1/hinton/Insecta/errors')
  _ = (good_input | 'Extract label ids' >> beam.ParDo(ExtractLabelIdsDoFn(),
...
def default_args(argv):
  ...
  parser.add_argument(
      '--job_name',
      type=str,
      default='insecta-order-' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S'),
