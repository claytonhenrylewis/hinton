...
declare -r JOB_ID="insects_genus_${USER}_$(date +%Y%m%d_%H%M%S)"
declare -r BUCKET="gs://bugoff-bucket1"
declare -r GCS_PATH="${BUCKET}/${USER}/${JOB_ID}"
declare -r DICT_FILE="${BUCKET}/hinton/Insecta/label.txt"

declare -r MODEL_NAME=insects-genus
...
python trainer/preprocess.py \
  ...
  --input_path "${BUCKET}/hinton/Insecta/eval_set.csv" \
  --output_path "${GCS_PATH}/preproc/eval" \
  --cloud

python trainer/preprocess.py \
  --input_dict "$DICT_FILE" \
  --input_path "${BUCKET}/hinton/Insecta/train_set.csv" \
